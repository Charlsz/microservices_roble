...
app.get("/", (req, res) => {
  res.json({
    message: "Microservicio USER activo. Usa los endpoints /login, /user, etc."
  });
});
...
app.post("/user", ensureAuth, async (req, res) => {
  ...
  try {
    const response = await axios.post(
      ...
      { tableName: "users", records: [req.body] },
...
app.get("/user", ensureAuth, async (req, res) => {
  try {
    const params = { tableName: "users", ...req.query };
    ...
    if (error.response?.status === 401) {
      ...
      const retryResp = await axios.get(`${BASE_URL}/database/${CONTRACT}/read`, {
        headers: { Authorization: `Bearer ${newToken}` },
        params: { tableName: "users", ...req.query }
      });
      return res.json(retryResp.data);
    }
    res
...
app.put("/user", ensureAuth, async (req, res) => {
  if (!req.body || Object.keys(req.body).length === 0)
    return res.status(400).json({ error: "Faltan datos en el cuerpo" });
...
app.delete("/user", ensureAuth, async (req, res) => {
  if (!req.body || Object.keys(req.body).length === 0)
    return res.status(400).json({ error: "Faltan datos en el cuerpo" });
  ...
  try {
    ...
    const response = await axios.delete(`${BASE_URL}/database/${CONTRACT}/delete`, {
      headers: { Authorization: `Bearer ${req.accessToken}` },
      data: { tableName: "users", idColumn: "_id", idValue }
    });
    res.json(response.data);
  } catch (error) {
    if (error.response?.status === 401) {
...
app.listen(PORT, () => console.log(`Microservicio USER corriendo en puerto ${PORT}`));
